...
\newcommand{\MonoLeadingRatio}{1.25}      % Iosevka Extended leading = 1250/1000
                                           % Vertical-spanning glyphs (│ ║ █ ░)
                                           % are exactly GridFontSize * this tall.
                                           % One grid row = one glyph height so
                                           % box-drawing chars tile with zero gaps.
...
%
%     CellHeight includes the font leading ratio so that one grid row
%     equals the full glyph height of vertical-spanning characters.
\newcommand{\PtToMM}{\fpeval{25.4 / 72.27}}% = 0.35146… mm per TeX pt
\newcommand{\CellWidth}{\fpeval{\GridFontSize * \MonoWidthRatio * \PtToMM}}
\newcommand{\CellHeight}{\fpeval{\GridFontSize * \MonoLeadingRatio * \PtToMM}}
...
\newcommand{\CellHeightPt}{\fpeval{\GridFontSize * \MonoLeadingRatio}}% pt (includes leading)
...
% --- Border row height (em-square only, no leading) ------------------------
%     Top/bottom border chars (┌ ─ ╖ ╘ ═ ╝) are em-square height, not
%     full-leading height like │ ║ █.  Use this for border rows so they
%     sit flush against the adjacent body rows.
\newlength{\BorderRowHeight}
% \setlength{\BorderRowHeight}{\GridFontSize pt}  % exact em-square height
\setlength{\BorderRowHeight}{0.98\dimexpr\GridFontSize pt\relax}

%     As a fraction of one grid row (for textpos coordinate math):
\newcommand{\BorderRowGrid}{\fpeval{1 / \MonoLeadingRatio}}% = 0.8 at 1.25
...
% --- Content line height ----------------------------------------------------
%     Body text baselineskip as a multiple of one grid row (\TPVertModule).
%     Grid row = GridFontSize * MonoLeadingRatio = 11.25pt (at 9pt/1.25).
%     1.0 = text baselineskip equals one grid row (11.25pt).
%     0.96 = 10.8pt baselineskip (matches the previous 1.2 × 9pt).
\newcommand{\ContentLeading}{0.96}
...
% --- Header (master) -------------------------------------------------------
%   All header themes have 6 rows.  Now that \TPVertModule includes the
%   font leading ratio (11.25pt at 9pt/1.25), every row — whether drawn
%   via \FullHRow or \vbox to \TPVertModule — occupies exactly 1 grid row.
...
% \FullHRow{content} — stack a row at its NATURAL glyph height.
%
%   Now that \TPVertModule = GridFontSize × MonoLeadingRatio (= 11.25pt
%   at 9pt/1.25), the natural glyph height equals one grid row, so
%   \FullHRow and \vbox to \TPVertModule produce the same physical height.
%   \FullHRow is kept for header templates that use block/shade chars
%   (█░▓║│▀▄) — it raises the content by its depth so all ink sits above
%   the baseline (dp→0, ht→ht+dp) and rows tile flush via
%   \offinterlineskip.
\newsavebox{\hrowbox}
